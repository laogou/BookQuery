...
public class AdminLoginFrame extends JFrame{
	...
	public void action(){
		...
		JLabel bookEdit = new JLabel("图书管理员入口");    //创建 JLabel 对象，用于短文本字符串或图像或二者的显示区
				...
				new ActionListener(){
					...
					public void actionPerformed(ActionEvent e){
						...
						if( nameField.getText() != null && nameField.getText().trim().length() != 0 ){
							admin = adminManager.findByAdminName( nameField.getText() );
						}
						
						if( admin.getAdminPassword().equals(passwordField.getText()) ){
							BookListFrame bookListFrame = new BookListFrame();
							
							bookListFrame.action();
							bookListFrame.createWindow();
						}else{
